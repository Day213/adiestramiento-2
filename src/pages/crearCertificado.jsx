...
export const CrearCertificado = () => {
  ...
  const [participantes, setParticipantes] = useState(() => {
    const saved = localStorage.getItem("certificado_participantes");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error("Error parsing certificado_participantes from localStorage", e);
      }
    }
    return [
  ...
  const [formData, setFormData] = useState(() => {
    const saved = localStorage.getItem("certificado_form_data");
    if (saved) {
      try {
        return JSON.parse(saved);
      } catch (e) {
        console.error("Error parsing certificado_form_data from localStorage", e);
      }
    }
    return {
      nombre_solicitud: "",
  ...
  useEffect(() => {
    localStorage.setItem("certificado_form_data", JSON.stringify(formData));
  }, [formData]);

  useEffect(() => {
    localStorage.setItem("certificado_participantes", JSON.stringify(participantes));
  }, [participantes]);
  ...
  const handleResetForm = () => {
    if (window.confirm("¿Está seguro de que desea limpiar todos los campos del formulario?")) {
      localStorage.removeItem("certificado_form_data");
      localStorage.removeItem("certificado_participantes");
      setFormData({
        nombre_solicitud: "",
        duracion: "",
        fecha_inicial: new Date().toISOString().split("T")[0],
              ...
              <div className="pt-8 w-full space-y-4">
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <button
                    ...
                    className="sm:col-span-2 flex justify-center items-center bg-slate-100 hover:bg-slate-200 text-slate-600 hover:text-slate-800 px-6 py-3.5 rounded-2xl font-bold text-sm sm:text-base transition-all transform active:scale-[0.98] border border-slate-200"
                  >
                    <span className="flex items-center gap-2">
                      Limpiar Formulario
                      <svg
